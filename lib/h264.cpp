...
// encodes a file into h264 with ideal config for messing with i-frames
// requires ffmpeg
// WARNING: can run arbitrary commands, input is NOT sanitized
int h264_enc(char *infile, char *outfile) {
  std::string command = "ffmpeg -i " + std::string(infile) +
                        " -an -vcodec libx264 -crf 23 " + std::string(outfile);
